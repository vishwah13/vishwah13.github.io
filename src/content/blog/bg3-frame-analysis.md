...
Baldur's Gate 3 draws this frame in 13,441 events. It submits its heaviest meshes three
times before evaluating a single light — Depth Pre-pass, G-Buffer, Velocity — and then once
more into each of six depth-only shadow passes. It spends **9.4 million triangles on shadows
against 2.6 million you can see**. Every one of its 2,601 draw calls is direct. Its
descriptor bindings are a hand-numbered slot map. It targets a Vulkan feature level that
predates the game by five years.

Read as a checklist, that looks like a renderer behind the times. Read with the reason
attached, it looks like a series of correct decisions.

The reason is that **this is a DirectX 11 renderer that also speaks Vulkan**. Vulkan arrived
because Google Stadia required it, on a schedule set by an early-access launch, added by an
engine team who joined after pre-production. DirectX 11 could not be dropped. Once one
renderer has to serve both APIs, bindless resources, `drawIndirectCount` and GPU-driven
geometry submission all stop being options — not because nobody thought of them, but because
each would mean maintaining a second, divergent path. The 1.1 floor, the numbered slot map,
the direct draws and the HLSL toolchain are the same decision seen from four angles.

And within that constraint the engineering is consistently sharp. Skinning runs once into a
buffer that eight later passes reuse. Fading objects dither through the **stencil** buffer so
no material needs a `discard` and early-Z survives, then a compute pass reconstructs the
image so the dither never shows. Gameplay surfaces build their decal mesh in **screen space**,
16×16 pixel tiles whose indices are generated by compute, so empty regions cost nothing.
Terrain is one instanced draw of a fixed 16,384-triangle patch, with holes punched by writing
**NaN** to a centre vertex. Lighting compiles into fourteen shader variations and a
classifier that hands each 8×8 tile to the cheapest one that fits — 95.7% of this frame runs
a shader that knows about exactly one shading model. And the volumetric fog, added late
because the game didn't look good enough without it, ends up lighting every particle in the
frame for free, because a volume that already knows how much light reaches each froxel is
most of what a particle needs.

None of those are exotic techniques. They are ordinary ones, chosen well and fitted to a
constraint.

### On being wrong

This study corrected itself repeatedly, and the corrections were more instructive than the
findings.

I called the renderer bindless because the SPIR-V declared `RuntimeDescriptorArray` — a
capability token DXC emits whether or not a shader uses it. I read MRT4 as a translucency
tint because it renders as vivid per-object colours, when it is four integers bit-packed
across four channels and the colours are bit patterns. I read a pass as emissive because it
re-rendered scene geometry and came out near-black, when its output is a `float2` and it is
velocity — near-black because the camera wasn't moving. I let a summary tool group two
unrelated systems and print one render-target size for both, and repeated that grouping as
fact for several sessions even after a second capture contradicted it.

Every one of those was a confident, coherent reading of real evidence. What broke them was
always the same move: stop inferring from what a buffer *looks* like and read what the shader
*writes*. A G-buffer channel is a place to put bits, not necessarily a picture.

### Still open

- MRT3 holds something motion-related that the static shader writes as literal zero, while a
  separate pass computes full screen-space velocity. The relationship is unconfirmed.
- The Grove capture's tile classifier covers 57,599 of 57,600 tiles. One tile, unaccounted
  for, in that scene only.
- The 84 inputs to the lighting composite are unidentified.
- The blur pyramid before the HUD is either late bloom or a UI backdrop; I never traced its
  consumer.

## Reading Material and References

Capture analysis is my own, performed with RenderDoc 1.45 and `rdc-cli` against three
Vulkan captures. External material, used only where attributed in the text:

- Wannes Vanderstappen, ["The Road to Baldur's Gate 3"](https://www.youtube.com/watch?v=zuDjcoabX7U),
  Graphics Programming Conference 2024. Segments consulted: tile-based shading
  classification, terrain, shadow mapping, sky and clouds, volumetric fog, fading opaque
  objects, and surface decal rendering.
- [GPC 2024 archive](https://graphicsprogrammingconference.com/archive/2024/) — talk abstract
- [80.lv — "Larian Studios' Dev Explained Why Baldur's Gate 3 Supports Two APIs"](https://80.lv/articles/baldur-s-gate-3-dev-explained-why-it-supports-two-apis) — the Stadia and DirectX 11 context
- Hawar Doghramachi, "Tile-based Omnidirectional Shadows", *GPU Pro 6* (2015) — the shadow
  atlas technique Larian cite
- Kircher & Lawrance, "Inferred Lighting: Fast dynamic lighting and shadows for opaque and
  translucent objects" (2009) — the discontinuity-sensitive filter that inspired the fade
  reconstruction

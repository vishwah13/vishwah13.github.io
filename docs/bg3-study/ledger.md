...
| 19 | **Blur pyramid** | 12835–12916 | **VERIFIED** | **NOT the 8192² atlas.** RTs form 1280×720 → 672×392 → 352×212 → 672×392 → 1280×720. Shader = 4 UV inputs, 4 samples of one texture, `* 0.25` — a **4-tap box filter** down then up a pyramid with alpha blending. **Blurs HUD elements, not the scene**: targets are almost entirely `UNDEFINED` except a region containing "Passives/Custom/Common/Wizard/Items", the coord readout and hotbar bars — and it runs *before* the HUD pass at 12921. Reading: **UI glow**. Consumer not traced; one pyramid level showed no UI content in the sampled region, unexplained | — |
